  ...
  $('.row').off('click', '.btnSubmit').on('click', '.btnSubmit', function () {
    ...
    $.ajax({
      ...
      .done(function (response) {
        ...
        if (status) {
          console.log(true, 'Επιτυχής εισαγωγή του προίόντος');
          alert(true, 'Επιτυχής εισαγωγή του προϊόντος');
          $('#frmProduct')[0].reset();
          window.location.replace("http://localhost:3000/product/find.html")
        } else {
          console.log(false, 'Πρόβλημα στην εισαγωγή του προίόντος (' + data.message + ')');
          alert(false, 'Πρόβλημα στην εισαγωγή του προίόντος (' + data.message + ')');
          $('#frmProduct')[0].reset();
